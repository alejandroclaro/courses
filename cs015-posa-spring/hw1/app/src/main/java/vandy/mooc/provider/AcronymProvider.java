    ...
    switch (sUriMatcher.match(uri))
    {
      case ACRONYMS:
        ...
        try
        {
          ...
          {
            final long id = db.insert(AcronymContract.AcronymEntry.TABLE_NAME, null, value);
                      ...
                      String sortOrder)
  {
    ...
    switch (sUriMatcher.match(uri))
    {
      ...
      case ACRONYM:
        ...
        // from Uri.
        final String rowId = "" + AcronymContract.AcronymEntry._ID + " = '" + ContentUris.parseId(uri) + "'";

        // TODO -- replace "null" by writing code to query the
        // SQLite database for the particular rowId based on (a
        // subset of) the parameters passed into the method.a
        selection = TextUtils.isEmpty(selection) ? selection = rowId : selection + " AND " + rowId;
