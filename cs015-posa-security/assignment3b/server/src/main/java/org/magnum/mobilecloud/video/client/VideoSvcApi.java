 ...
 * https://github.com/juleswhite/mobilecloud-14/tree/master/examples/9-
 * VideoServiceWithOauth2/src/main/java/org/magnum/mobilecloud/video/auth You
 * will need to @Import the OAuth2SecurityConfiguration into your Application or
 * other configuration class to enable OAuth 2.0. You will also need to remove
 * one of the containerCustomizer() methods in either
 * OAuth2SecurityConfiguration or Application (they are the exact same code).
 * You may need to customize the users in the OAuth2Config constructor or the
 * security applied by the ResourceServer.configure(...) method. You should
 * determine what (if any) adaptations are needed by comparing this and the test
 * specification against the code in that class.
 *
