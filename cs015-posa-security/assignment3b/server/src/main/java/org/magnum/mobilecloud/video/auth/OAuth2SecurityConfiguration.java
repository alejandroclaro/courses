...
public class OAuth2SecurityConfiguration
{
  ...
  protected static class OAuth2Config extends AuthorizationServerConfigurerAdapter
  {
    ...
    /**
     *
     * This constructor is used to setup the clients and users that will be able
     * to login to the system. This is a VERY insecure setup that is using
     * hard-coded lists of clients / users / passwords and should never be used
     * for anything other than local testing on a machine that is not accessible
     * via the Internet. Even if you use this code for testing, at the bare
     * minimum, you should consider changing the passwords listed below and
    ...
    public OAuth2Config() throws Exception
    {
      ...
      ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
          ...
          // video service
          .withClient("mobile").authorizedGrantTypes("password").authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
          .scopes("read", "write").resourceIds("video").and()
          // Create a second client that only has "read" access to the
          // video service
          .withClient("mobileReader").authorizedGrantTypes("password").authorities("ROLE_CLIENT").scopes("read")
          .resourceIds("video").accessTokenValiditySeconds(3600).and().build();

      // Create a series of hard-coded users.
      UserDetailsService svc = new InMemoryUserDetailsManager(
          Arrays.asList(User.create("admin", "pass", "ADMIN", "USER"), User.create("user0", "pass", "USER")));
