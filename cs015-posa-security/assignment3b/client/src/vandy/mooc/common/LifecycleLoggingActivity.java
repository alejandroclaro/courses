...
public abstract class LifecycleLoggingActivity extends Activity
{
  ...
  /**
   * Hook method called after onCreate() or after onRestart() (when the activity
   * is being restarted from stopped state). Should re-acquire resources
   * relinquished when activity was stopped (onStop()) or acquire those
   * resources for the first time after onCreate().
   */
  @Override
  ...
  /**
   * Hook method called after onRestoreStateInstance(Bundle) only if there is a
   * prior saved instance state in Bundle object. onResume() is called
   ...
   * Save persistent state (onSaveInstanceState()) in case app is killed. Often
  ...
  /**
   * Called when Activity is no longer visible. Release resources that may cause
   * memory leak. Save instance state (onSaveInstanceState()) in case activity
   * is killed.
   */
  @Override
  ...
  /**
   * Hook method called when user restarts a stopped activity. Is followed by a
   * call to onStart() and onResume().
   */
  @Override
  ...
  /**
   * Hook method that gives a final chance to release resources and stop spawned
   * threads. onDestroy() may not always be called-when system kills hosting
   * process
   */
  @Override
