...
/**
 * The base interface that an operations ("Ops") class can implement so that it
 * can be notified automatically by the GenericAsyncTask framework during the
 * AsyncTask processing.
 */
public interface GenericAsyncTaskOps<Params, Progress, Result>
{
  /**
   * Called in the UI thread prior to running doInBackground() in a background
   * thread.
   */
